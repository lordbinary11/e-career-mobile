...
try {
    ...
    error_log("get_counselor_meetings.php: Looking for meetings with counselor_email = " . $counselor_email);
    ...
    // Debug: Also log all meetings in the database
    $debug_stmt = $pdo->prepare("SELECT counselor_email, COUNT(*) as count FROM scheduled_meetings GROUP BY counselor_email");
    $debug_stmt->execute();
    $all_counselors = $debug_stmt->fetchAll(PDO::FETCH_ASSOC);
    error_log("get_counselor_meetings.php: All counselors with meetings: " . json_encode($all_counselors));
    $meetings = $stmt->fetchAll(PDO::FETCH_ASSOC);

    error_log("get_counselor_meetings.php: Found " . count($meetings) . " meetings for counselor " . $counselor_email);
    if (count($meetings) > 0) {
        error_log("get_counselor_meetings.php: First meeting details: " . json_encode($meetings[0]));
    }
