...
try {
    ...
    if ($is_counselor_inbox_request) {
        ...
        // No additional validation needed - the authenticated counselor ID is used directly
    } else {
    ...
    // This requires the is_counselor_inbox flag and the authenticated counselor's ID.
    else if ($is_counselor_inbox_request && $authenticated_role === 'counselor') {
        $sql = "
            SELECT
                m.id,
                m.user_id,
