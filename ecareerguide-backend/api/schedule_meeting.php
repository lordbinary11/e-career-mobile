...
try {
    ...
    // Virtual meeting properties
    $is_virtual_meet = $data['is_virtual_meet'] ?? false;
    ...
    // Validate virtual meeting data
    if ($is_virtual_meet && (!$meeting_platform || !$meeting_link)) {
        http_response_code(400);
        echo json_encode(["success" => false, "error" => "Meeting platform and link are required for virtual meetings."]);
        exit();
    }
    ...
    $stmt_meeting = $pdo->prepare("
        INSERT INTO scheduled_meetings (user_email, counselor_email, schedule_date, schedule_time, purpose, is_virtual_meet, meeting_platform, meeting_link)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt_meeting->execute([$user_email, $counselor_email, $schedule_date, $schedule_time, $purpose, $is_virtual_meet, $meeting_platform, $meeting_link]);
